                ...
                (globalLoading || countryLoading) ?
                    ...
                    <View style={{ paddingTop: Platform.OS === 'android' ? StatusBar.currentHeight : 0 }}>
                        ...
                        <Block center fluid style={{ height: 250, justifyContent: 'center' }}>
                            ...
                            <Text color={'rgb(60,179,113)'} h5>{confirmed}</Text>
                            <Text bold size={20}><Icon name="trending-up" family="Feather" size={12} /> Recovered</Text>
                            <Text color={'rgb(0,191,255)'} h5>{recovered}<Text muted size={16}>({((recovered / confirmed) * 100).toFixed(2)}%)</Text></Text>
                            <Text bold size={20}><Icon name="trending-down" family="Feather" size={12} /> Death </Text>
                            <Text color={'rgb(255,69,0)'} h5>{death}<Text muted size={16}>({((death / confirmed) * 100).toFixed(2)}%)</Text></Text>
                        </Block>
                        <Block center fluid style={{ height: 250, justifyContent: 'center' }}>
                            <Button round color="#50C7C750" style={{ width: 300 }}><Text p bold> INDONESIA</Text></Button>
                            <Text bold size={20}><Icon name="users" family="Feather" size={12} /> Cases </Text>
                            <Text color={'rgb(60,179,113)'} h5>{localConfirmed}</Text>
                            <Text bold size={20}><Icon name="trending-up" family="Feather" size={12} /> Recovered</Text>
                            <Text color={'rgb(0,191,255)'} h5>{localRecovered}<Text muted size={16}>({((localRecovered / localConfirmed) * 100).toFixed(2)}%)</Text></Text>
                            <Text bold size={20}><Icon name="trending-down" family="Feather" size={12} /> Death </Text>
                            <Text color={'rgb(255,69,0)'} h5>{localDeath}<Text muted size={16}>({((localDeath / localConfirmed) * 100).toFixed(2)}%)</Text></Text>
                        </Block>
                    </View>
            }
        </View >
